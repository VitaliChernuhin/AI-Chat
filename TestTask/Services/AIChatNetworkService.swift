...
protocol AIChatNetworkService {
    ...
    /// Получение сообщений чата пользователя
    func fetchMessages(
        chatId: String,
        userId: String,
        appId: String,
        chatsLimitCount: Int?,
        chatsPaginationOffset: Int?
    ) -> AnyPublisher<[AIChatHistoryMessageResponse], Error>
}
...
final class AIChatNetworkServiceImpl: AIChatNetworkService {
    ...
    func chatsList(
        ...
        .decode(type: [AIChatHistoryResponse].self, decoder: decoder)
        .mapError { [weak self] error -> Error in
            return self?.mapNetworkError(error) ?? AIChatError.unknown(error.localizedDescription)
        }
        .eraseToAnyPublisher()
    ...
    func fetchMessages(
        ...
        .decode(type: [AIChatHistoryMessageResponse].self, decoder: decoder)
        .mapError { [weak self] error -> Error in
            return self?.mapNetworkError(error) ?? AIChatError.unknown(error.localizedDescription)
        }
        .eraseToAnyPublisher()
...
private extension AIChatNetworkServiceImpl {
    ...
    func handleResponseData(_ response: Response) throws -> Data {
        ...
        case 422:
            // Распаковываем ошибку валидации со всеми нашими новыми динамическими полями
            if let errorResponse = try? decoder.decode(AIValidationErrorResponse.self, from: response.data),
               let firstDetail = errorResponse.detail.first {
                throw AIChatError.validation(firstDetail)
            }
            throw AIChatError.unknown("Неизвестная ошибка валидации (422)")
